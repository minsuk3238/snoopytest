  ...
  },
  {
    ...
    path: [
      ...
      },
      {
        ...
        text: "다른 애들 개는 평범하던데, 왜 내 개만 이럴까?",
        source: "- 찰리 브라운 (1974.10.05.)",
      },
      {
        type: "location",
        name: "찰리 브라운의 야구장 담장",
      ...
      },
      {
        ...
        text: "투수 마운드에 서면, 세상에서 가장 외로운 사람이 된 기분이야.",
        source: "- 찰리 브라운 (1956.04.13.)",
      },
      {
        type: "location",
        name: "피너츠 컬러가든",
...
export default function App() {
  ...
  const handleScanQR = () => setStep("qr");
  const handleQRSuccess = () => setStep("scene");
              ...
              <div className="w-full bg-white rounded-2xl border border-stone-200 p-5 mb-10 text-left shadow-sm">
                ...
                <div className="space-y-4">
                  {activeTheme.completion.dialogues.map((dialogue, idx) => (
                    <div
                      key={idx}
                      className="bg-stone-50 p-4 rounded-xl border border-stone-200 flex items-start gap-3 shadow-sm"
                    >
                      <Quote className="w-4 h-4 text-stone-300 shrink-0 mt-0.5" />
                      <div>
                        <p className="text-stone-800 font-bold text-[13px] leading-relaxed break-keep">
                          "{dialogue.text}"
                        </p>
                        {dialogue.date && (
